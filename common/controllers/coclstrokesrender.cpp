...
OCLStrokesRenderImpl::OCLStrokesRenderImpl()
{
}


OCLStrokesRenderImpl::OCLStrokesRenderImpl(const StrokesTemplateData &tmpl_data) : _tmpl_data(tmpl_data)
{
}
...
OCLStrokesRenderImpl::~OCLStrokesRenderImpl()
{
  if (_ocl_command_queue)
    delete _ocl_command_queue;
...
bool OCLStrokesRenderImpl::initContext(QOpenGLContext *owner_cntx)
{
...
bool OCLStrokesRenderImpl::createLinearTrack(
...
float OCLStrokesRenderImpl::length(cl_float2 f)
{
...
int OCLStrokesRenderImpl::createStrokeCL(
    const QTransform &in_to_scene_tx,
    const QRectF &in_rect,
    const QList<KnifeRec> &in_knives)
{
...
/*
  ...
  command_queue = new clCommandQueue(clCreateCommandQueue(_ocl_context->context, device_id, 0, &ret));
  ...
  program = new clProgram(clCreateProgramWithSource(_ocl_context->context, 1, (const char **)&kernelSource_ptr,
                                                    (const size_t *)&source_size, &ret));
  ret = clBuildProgram(program->program, 1, &device_id, NULL, NULL, NULL);
  if (ret != CL_SUCCESS)
  {
    char buffer[0x100000];
    clGetProgramBuildInfo(program->program, device_id, CL_PROGRAM_BUILD_LOG, sizeof(buffer), buffer, NULL);
...
void OCLStrokesRenderImpl::_init_painter_data()
{
...
int OCLStrokesRenderImpl::render()
{
  _ogl_context->makeCurrent(_offscrnsf);

  cl_int ret = _render();

  _ocl_command_queue->finish();
...
void OCLStrokesRenderImpl::TIME_LOG(const QString &txt)
{
#ifdef RENDER_DEBUG_INFO
