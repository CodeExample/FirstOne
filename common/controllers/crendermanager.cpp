...
void CRenderManager::append(const COCLRenderThreadTask &render_task)
{
  QMutexLocker l(&mutex);
  if (!render_thread)
  {
    COCLStrokesRender strokes_render(true);
    strokes_render.initContext(hprj->context());
    strokes_render.initKernel();

    render_thread = new COCLRenderThread(this);
    render_thread->init(strokes_render);

    connect(render_thread, SIGNAL(knifeEdgesCreated(int, StrokesTemplateData*)), this,
            SLOT(_knife_edges_created(int, StrokesTemplateData*)), Qt::BlockingQueuedConnection);
...
void CRenderManager::_knife_edges_created(int tmpl_id, StrokesTemplateData * tmpl_data)
{
