...
#ifdef sse_test
    //calculateBezier(&points0[0], &points1[0], t, &p10p12[0]);
    //qDebug() << "b - a" << p10p12[0] << p10p12[1] << p10p12[2] << p10p12[3];
  // sse
  {
    float t = 0.5;
    //                  p0   p1   p2   p3
    ...
    float sum_pc_t=0;
    QTime tm; tm.start();
    /*__m128 dddd = _mm_set1_ps(1e-5);
    __m128 zero = _mm_set1_ps(0);
    __m128 tttt = _mm_set1_ps(t);
    __m128 rslt = _mm_set1_ps(0);

    __m128 x0_y0_x1_y1 = _mm_load_ps(&points0[0]);
    __m128 x1_y1_x2_y2 = _mm_load_ps(&points1[0]);
    __m128 ax_ay_bx_by, b_sub_a;*/
    for(int i=0; i < 100000000; ++i)
    {
      ...
      sum_pc_t += p10p12[0];
      sum_pc_t += p10p12[1];
/*      tttt = _mm_add_ps(tttt, dddd);

      ax_ay_bx_by = _mm_add_ps(x0_y0_x1_y1,
                                      _mm_mul_ps(
                                        _mm_sub_ps(x1_y1_x2_y2, x0_y0_x1_y1), tttt));

      b_sub_a = _mm_mul_ps(_mm_hsub_ps(zero, _mm_shuffle_ps(ax_ay_bx_by, ax_ay_bx_by, 0b00100111)), tttt);
      rslt = _mm_add_ps(rslt, b_sub_a);*/
    }
    //_mm_store_ps(&p10p12[0], rslt);
    qDebug() << "sse_pc_t" << sum_pc_t << tm.elapsed();
  }
  ...
  // proc
  {
    ...
    qDebug() << "sum_pc_t" << sum_pc_t << tm.elapsed();
  }
  ...
  qDebug() << " proc_a" << proc_a << t.elapsed();
*/
